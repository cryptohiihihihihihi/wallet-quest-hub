...
export function BuyDialog({ product }: { product: Product }) {
  ...
  return (
    <Dialog onOpenChange={() => { setConfirmed(false); setChain(chains[0]); setEmail(""); setEmailError(null); setPaymentMethod("crypto"); }}>
        ...
        {!confirmed ? (
          <div className="space-y-4">
            ...
            {paymentMethod === "crypto" ? (
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-border/50 bg-secondary/30 p-3.5">
                  ...
                  <Input placeholder="1234 5678 9012 3456" className="h-10 bg-secondary/40 border-border/60" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl border border-border/50 bg-secondary/30 p-3.5">
                    <div className="mb-2 text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Expiry</div>
                    <Input placeholder="MM / YY" className="h-10 bg-secondary/40 border-border/60" />
                  </div>
                  <div className="rounded-xl border border-border/50 bg-secondary/30 p-3.5">
                    <div className="mb-2 text-[10px] uppercase tracking-[0.15em] text-muted-foreground">CVC</div>
                    <Input placeholder="123" className="h-10 bg-secondary/40 border-border/60" />
                  </div>
                </div>
                <div className="rounded-xl border border-border/50 bg-secondary/30 p-3.5">
                  <div className="mb-2 text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Cardholder name</div>
                  <Input placeholder="Jane Doe" className="h-10 bg-secondary/40 border-border/60" />
                </div>
                <div className="flex items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-muted-foreground">
                  ...
                  Card payments UI preview only - no transaction processing yet.
                </div>
              </div>
            )}
            ...
            <Button variant="hero" size="lg" className="w-full" onClick={handleConfirm}>
              {paymentMethod === "crypto" ? "I've sent the payment" : "Pay with card (preview)"}
            </Button>
          </div>
        ) : (
